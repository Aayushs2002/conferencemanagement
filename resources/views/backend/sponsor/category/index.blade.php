        ...
        <div class="card-datatable table-responsive pt-0">
            <div class="row card-header flex-column flex-md-row border-bottom mx-0 px-3">
                <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto mt-0">
                    <h5 class="card-title mb-0 text-md-start text-center pb-md-0 pb-6">Sponsor Category</h5>
                </div>
                <div class="d-md-flex justify-content-between align-items-center dt-layout-end col-md-auto ms-auto mt-0">
                    <div class="dt-buttons btn-group flex-wrap mb-0">
                        ...
                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Add Sponsor Category'))
                            <a href="{{ route('sponsor-category.create', [$society, $conference]) }}"
            ...
            <div class="px-3 py-2">
                <small class="text-info"> 
                    <i class="ti tabler-grip-vertical"></i> <strong>Drag and drop rows to reorder sponsor categories</strong>
                </small>
            </div>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Edit Sponsor Category'))
                                            <a class="dropdown-item"
                                                href="{{ route('sponsor-category.edit', [$society, $conference, $category->id]) }}"><i
                                                    class="icon-base ti tabler-pencil me-1"></i> Edit</a>
                                        @endif
                                        <hr>
                                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Delete Sponsor Category'))
                                            <form
                                                action="{{ route('sponsor-category.destroy', [$society, $conference, $category->id]) }}"
                                                method="POST">
                                                @method('delete')
                                                @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateOrder() {
                ...
                $.ajax({
                    url: '{{ route('sponsor-category.update-order', [$society, $conference]) }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        orders: orders
