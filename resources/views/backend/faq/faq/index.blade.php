        ...
        <div class="card-datatable table-responsive pt-0">
            <div class="row card-header flex-column flex-md-row border-bottom mx-0 px-3">
                <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto mt-0">
                    <h5 class="card-title mb-0 text-md-start text-center pb-md-0 pb-6">Faq</h5>
                </div>
                <div class="d-md-flex justify-content-between align-items-center dt-layout-end col-md-auto ms-auto mt-0">
                    <div class="dt-buttons btn-group flex-wrap mb-0">
                        ...
                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Add Faq'))
                            <a href="{{ route('faq.create', [$society, $conference]) }}" class="btn btn-primary"
                                tabindex="0">
            ...
            <div class="px-3 py-2">
                <small class="text-info"> 
                    <i class="ti tabler-grip-vertical"></i> <strong>Drag and drop rows to reorder FAQs</strong>
                </small>
            </div>
                        ...
                        <tr data-id="{{ $faq->id }}" style="cursor: move;">
                            ...
                            <td>
                                @if ($faq->visible_status == 1)
                                    <a href="{{ route('faq.changeStatus', $faq->id) }}" class="btn btn-success ml-2"
                                        title="Unpublish Faq"><i class="icon-base ti tabler-circle-check"></i></a>
                                @else
                                    <a href="{{ route('faq.changeStatus', $faq->id) }}" class="btn btn-danger ml-2"
                                        title="Publish Faq"><i class="icon-base ti tabler-circle-x"></i></a>
                                @endif
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Edit Faq'))
                                            <a class="dropdown-item"
                                                href="{{ route('faq.edit', [$society, $conference, $faq->id]) }}"><i
                                                    class="icon-base ti tabler-pencil me-1"></i> Edit</a>
                                        @endif
                                        <hr>
                                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Delete Faq'))
                                            <form action="{{ route('faq.destroy', [$society, $conference, $faq->id]) }}"
                                                method="POST">
                                                @method('delete')
                                                @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateOrder() {
                ...
                $.ajax({
                    url: '{{ route('faq.update-order', [$society, $conference]) }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        orders: orders
