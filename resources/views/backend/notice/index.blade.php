        ...
        <div class="card-datatable table-responsive pt-0">
            <div class="row card-header flex-column flex-md-row border-bottom mx-0 px-3">
                <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto mt-0">
                    <h5 class="card-title mb-0 text-md-start text-center pb-md-0 pb-6">News/Notice</h5>
                </div>
            ...
            <table class="datatables-basic table">
                <thead>
                    <tr>
                        ...
                        <th>Attachment</th>
                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Change Featured News/Notice'))
                            <th>Is Featured?</th>
                        @endif
                        <th>Action</th>
                    </tr>
                            ...
                            @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Change Featured News/Notice'))
                                <td>
                                    @if ($notice->is_featured == 1)
                                        <a href="{{ route('notice.changeFeatured', $notice->id) }}"
                                            class="btn btn-success ml-2" title="Unpublish News/Notice"><i
                                                class="icon-base ti tabler-circle-check"></i></a>
                                    @else
                                        <a href="{{ route('notice.changeFeatured', $notice->id) }}"
                                            class="btn btn-danger ml-2" title="Publish News/Notice"><i
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item viewData" data-id="{{ $notice->id }}"
                                            data-bs-toggle="modal" data-bs-target="#pricingModal"><i
                                                class="icon-base ti tabler-eye me-1 "></i> View</a>
                                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Edit News/Notice'))
                                            <a class="dropdown-item"
                                                href="{{ route('notice.edit', [$society, $conference, $notice->id]) }}"><i
                                                    class="icon-base ti tabler-pencil me-1"></i> Edit</a>
                                        @endif
                                        <hr>
                                        @if (auth()->user()->hasConferencePermissionBlade(getConference(request()->segment(4)), 'Delete News/Notice'))
                                            <form
                                                action="{{ route('notice.destroy', [$society, $conference, $notice->id]) }}"
                                                method="POST">
                                                @method('delete')
                                                @csrf
