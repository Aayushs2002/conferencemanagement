...
@section('content')
    <div class="col-md">
        <div class="card">
            <h4 class="card-header"><a href="{{ route('submission.index', [$society, $conference]) }}"><i
                        class="ti tabler-arrow-narrow-left"></i></a>
                {{ isset($submission) ? 'Edit' : 'Add' }}
                Presentation Submission</h4>
            <div class="card-body">
                <form class="needs-validation"
                    action="{{ isset($submission) ? route('submission.update', [$society, $conference, $submission]) : route('my-society.conference.submission.store', [$society, $conference]) }}"
                    ...
                    <div class="row">
                        ...
                        @if (!empty($collaborativePartners) && count($collaborativePartners) > 0)
                            <div class="mb-6 col-md-6">
                                <label for="collaborative_partner" class="form-label">Collaborative Partner</label>
                                <select class="form-select @error('collaborative_partner') is-invalid @enderror"
                                    name="collaborative_partner" id="collaborative_partner">
                                    <option value="">-- Select Collaborative Partner --</option>
